...
public:
	...
	static bool undistorPixcor(const vector<Point2f>& src,vector<Point2f>& dst,Mat& intrinsicPara,Mat& distCoeffs);
    ...
    ///output  type：方法，type=1时为双相机，type=0时为单相机
    ///备注：该函数用于根据输入两张左右相机图片获取标靶一号面在左相机坐标系下的位置姿态，可以用两种方式获得：单相机方式pnp的方法，双相机的方法。
    ///对于单相机，只要保证其中一个相机右图像即可，对于双相机一定要保证左右相机都有图片，并且能识别到同一个面
    static bool GetObjectRT(const Mat &LeftImage,const Mat &RightImage,const CamPara &L_Camparam,
        const CamPara &R_Camparam,const Mat &CamRT,const ObjectPara &objparam,Mat &RT2cam,
        std::string &err_inf,const int type=1);
...
//                        vector<double> &dpdrot,vector<double> &dpdt,vector<Point3f> &PointToCam);
	...
	///提取激光中心
	///@param image 输入 激光图像
	///@param mask  输入 mask,当前坐标位置的相位是否有效，1为有效，0为无效
	///@param Threshold 输入 激光图像
	///@param windowwidth  输入 窗口宽度大概是条纹宽度w的3倍, 为奇数
	///@param LaserCenter	输出 激光中心
	static bool LaserCenterDetector(const Mat& srcimage, const Rect mask, const int Threshold, const int windowwidth, vector<Point2f> &LaserCenter);

	//////////////////////////////         庄磊磊                 ///////////////////////////////////////////////////////////////////////    
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///提取激光中心----重载上面的函数
	///@param image 输入 激光图像
	///@param Threshold 输入 阈值---条纹亮度和背景亮度的差值
	///@param windowwidth  输入 窗口宽度大概是条纹宽度w的3倍, 为奇数
	///@param LaserCenter	输出 激光中心
	static bool LaserCenterDetector(const Mat& srcimage,const int Threshold, const int windowwidth, vector<Point2f> &LaserCenter);

	///提取激光中心----重载上面的函数，加入ROI
	///@param image 输入 激光图像
	///@param Threshold 输入 阈值---条纹亮度和背景亮度的差值
	///@param windowwidth  输入 窗口宽度大概是条纹宽度w的3倍, 为奇数
	///@param LaserCenter	输出 激光中心
	static bool LaserCenterDetector(const Rect ROI,Mat& srcimage, const int Threshold, const int windowwidth, vector<Point2f> &LaserCenter,Mat& gaussK);

	//对图像中的某一行指定的区域进行高斯滤波
	///@param srcImg	源图像
	///@param rowCurrent	当前行
	///@param colStart	开始列
	///@param colEnd	结束列
	///@param gaussK	高斯核
	static bool PartGaussianBlur(Mat& srcImg, int rowCurrent, int colStart, int colEnd, Mat& gaussK);

	//对图像中的某一行指定的区域进行5*5均值滤波
	///@param srcImg	源图像
	///@param rowCurrent	当前行
	///@param colStart	开始列
	///@param colEnd	结束列
	static bool PartMeanBlur(Mat& srcImg, int rowCurrent, int colStart, int colEnd);

	///提取激光中心----重载上面的函数，加入ROI;加入梯形搜索域来对算法进行加速
	///@param image 输入 激光图像
	///@param Threshold 输入 阈值---条纹亮度和背景亮度的差值
	///@param windowwidth  输入 窗口宽度大概是条纹宽度w的3倍, 为奇数
	///@param LaserCenter	输出 激光中心
	///@param searchWinHei  梯形搜索域的高度
	///@param gaussK	高斯卷积核
	static bool LaserCenterDetector(const Rect ROI, Mat& src, const int Threshold, const int windowwidth, vector<Point2f> &LaserCenter, int searchWinHei, Mat& gaussK);
	
	//计算行图像中指定列范围中的最大值
	static bool getRowMaxLoc(Mat& rowMat, int colStart, int colEnd, int& maxValue, int& colMax);

	//该函数在梯形域的基础上加入了centerRange,在求条纹首点时使用比较大的阈值以确保激光条纹的骨架是绝对正确的
	//@param  centerRange 图片中激光条纹列的最大间隔
	static bool LaserCenterDetector(cv::Rect ROI, Mat& srcimage, const int Threshold, const int windowwidth, int centerRange, vector<Point2f> &LaserCenter, int searchWinHei, Mat& gaussK);

	////////////////////////////// 庄磊磊 /////////////////////////////////////// 
	/////////////////////////////////////////////////////////////////////////////
	//计算点到直线的距离
	//@param coeffs 输入 二维直线的系数a b c
	//@param coeffs 输入 二维点
	//@param 输出 距离
	static float Point2LineDistance(Vec3f& coeffs,Point2f& point);

	//TODO:需要修改回来的地方 public 改回 private
public: 
    ///移除重复的中心点
