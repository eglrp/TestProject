...
,double alfpha_c )
{
    ...
    if (norm2>0)
    {
        ...
        {
            for (int j=0;j<20;j++)///迭代２０次求解非畸变量
            {
                                    ...
                                    vector<Point2f>& plane2dPnts, int& row, int& col)
{
    ...
    //cv::GaussianBlur(imgGray_ROI,imgGray_ROI,cv::Size(5,5),0);
    //chengwei changed
    ...
    for( vector< vector<Point> >::iterator itr=contours.begin();itr!=contours.end();++itr)
    {
        ...
        if ( height>width? height/width<2.5 : width/height<2.5 )
        {
            ...
            if(!CoreAlgorithm::findEllipses(imgroi2,ROIsingle,findResulttemp,0.05,1,3))
            {
                plane2dPnts.push_back(rotatedBox.center);
                continue;
            }
...
bool CoreAlgorithm::PlaneFitting(vector<Point3f> obsPoints,SPlane& model)
{
...
bool CoreAlgorithm::CalPlaneLineIntersectPoint(const SPlane _plane,const Line _line,Point3f &IntersectPoint)
{
...
float CoreAlgorithm::distancePlanes(const SPlane plane1,const SPlane plane2)
{
...
//重载上述函数，用于获取所有的轮廓的拟合椭圆
bool CoreAlgorithm::DiscernLSCircle(Mat img, Mat &imge_contour, vector<SH_Ellipse> &Fea_circle,  int Threshold, float rate)
{
	//input check
	if (img.channels() > 1)
	{
	...
	if (!contours.empty() && !hierarchy.empty())
	{
		...
		for (; idx >= 0; idx = hierarchy[idx][0])
		{
			...
			try
			{
				...
				{
					_rotatebox = fitEllipse(contours[idx]);
					if ((_rotatebox.size.height / _rotatebox.size.width) < rate)//第二个筛选条件：椭圆的长短轴的比值，去掉偏转角度较大的面（1.732为三条棱在投影平面上成120度时，的长短轴比）
					{
						//...
						rotatedBox.push_back(_rotatebox);
						cv::ellipse(imge_contour, _rotatebox, color, 3);
					}
				}
			}
			catch (...)		//三个点表示任何异常
...
bool CoreAlgorithm::FindMarkArea(Mat img,Mat& outimage,vector<SH_Ellipse> Fea_circle,MarkArea& MarkedArea,string& err_str)
{
	...
	{
		split(outimage, _temout);
		img.copyTo(_temout[1]);
		img.copyTo(_temout[2]);
		img.copyTo(_temout[0]);
		merge(_temout, outimage);
	}
    err_str="No Error!";
...
bool CoreAlgorithm::TransferFrame(const MarkArea& markedarea,const Mat& distCoeffs,const Mat& cameraMatrix,const ObjectPara& objparam, Mat& rtTOcam,string& err_str)
{
    ...
    for (int i=0;i<markedarea.size();i++)
    {
		...
		//若找到一号面，则直接返回
		if (markedarea[i].Face == Face_1)
		{
			return true;
		}
        //若不是一号面则进行坐标系转化，转化成一号面的位姿
...
bool CoreAlgorithm::GetObjectRT(const Mat &LeftImage,const Mat &RightImage, const CamPara &L_Camparam,
                                const CamPara &R_Camparam,const Mat &CamRT,const ObjectPara &objparam,Mat &outL, Mat &outR, Mat &RT2cam, std::string &err_inf)
{
    ...
    {
        bool flag_1,flag_2;
        flag_1=FindMarkArea(LeftImage,outL,Lcircle,L_markarea,err_inf);
        flag_2=FindMarkArea(RightImage,outR,Rcircle,R_markarea,err_inf);
        ...
        for (int i=0;i<L_markarea.size();i++)
        {
            for (int j=0;j<R_markarea.size();j++)
            {
                if (L_markarea[i].Face==R_markarea[j].Face)
                {
					...
					//将标定板的特征点投影到图像中，以检查位姿的准确性
					Mat R, T;
					vector <Point2f> pntInImage;
					Rodrigues(RT2cam.rowRange(0, 3).colRange(0, 3), R);
					T = RT2cam.rowRange(0, 3).colRange(3, 4);
					//有待修改，暂时投影一号面上的点
					projectPoints(objparam.FacePnts[0], R, T, L_cameraMatrix, L_distCoeffs, pntInImage);
					for (int i = 0; i < pntInImage.size(); i++)
					{
						DrawCircle(outL, Point2i((int)pntInImage[i].x, (int)pntInImage[i].y), 15);
					}
...
//描述：单相机获取标靶位姿
bool CoreAlgorithm::GetObjectRT(const Mat &Image, const CamPara &Camparam, const ObjectPara &objparam, Mat &outImage, Mat &RT2cam, std::string &err_inf)
{
	err_inf = "OK_come from GetObjectRT()";
	//解析相机参数
	Mat distCoeffs, cameraMatrix;
	if (!ExtraCamMatrixAndDist(Camparam, cameraMatrix, distCoeffs))
	{
		err_inf = "Error from ExtraCamMatrixAndDist()";
		return false;
	}
	...
	//进行大小圆提取
	bool flag = false;
	flag = DiscernLSCircle(Image, Lcircle, imge_contour);
	if (flag == false)
	{
		err_inf = "Error from GetObjectRT(): Both of two image can't discern right feature!";
		return false;
	...
	{
		if (FindMarkArea(Image, outImage, Lcircle, markarea, err_inf))
		{
			if (TransferFrame(markarea, distCoeffs, cameraMatrix, objparam, RT2cam, err_inf))
			{
				//将标定板的特征点投影到图像中，以检查位姿的准确性
				Mat R, T;
				vector <Point2f> pntInImage;			
				...
				T = RT2cam.rowRange(0, 3).colRange(3, 4);
				//有待修改，暂时投影一号面上的点
				projectPoints(objparam.FacePnts[0], R, T, cameraMatrix, distCoeffs, pntInImage);
				for (int i = 0; i < pntInImage.size();i++)
				{
					DrawCircle(outImage, Point2i((int)pntInImage[i].x, (int)pntInImage[i].y), 15);
				}
				return true;
			}
...
bool CoreAlgorithm::DrawCircle( Mat & inoutimage, const  Point2i & Center, const int & radius)
{
	...
	for (int i = Center.x - radius-offset; i < Center.x + radius+offset; i++)
	{
		for (int j = Center.y - radius - offset; j<Center.y + radius+offset; j++)
		{
			...
			if ( dist <= radius )
			{
				if (inoutimage.channels()>1)
				{
					inoutimage.at<cv::Vec3b>(j, i) = cv::Vec3b(0,0,255);
				}
				else
					inoutimage.at<uchar>(j,i) = 255;
			}
		}
	}
	return true;
