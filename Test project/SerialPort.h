...
public:
	...
	*
	*  
	*  @param:  char pData[] 输出的char数组指针
	*  @param:  unsigned int length 需要读取的数据长度
	*  @param:  UINT timeout 读取等待超时时间
	*  @return: bool  操作是否成功
	*  @note:   length不要大于pData数组大小
	*  @see:
	*/
	bool ReadData(char pData[], unsigned int length);
	...
	*/
	...
	/** 读取串口接收缓冲区中一个字节的数据(串口数据缓冲区，取一个少一个)
	*
	*
