#ifndef LASERSENSORALGORITHM_H
#define LASERSENSORALGORITHM_H
#include <cv.h>
#include "cvtool.h"
#include<string>
...
public:
	...
	//重载：不考虑高度变化
	cv::Point2f getXY(const cv::Point2f pointInImg, const cv::Point2f tcp);
	//图像点相对光心的物理尺寸偏置
	cv::Point2f getOffsetToCenter(const cv::Point2f pointInImg);
	//标定激光条纹成像的平移量和世界坐标Z轴移动量的比例关系
	//laserImg中需有四张激光图像，第一张为参考位置处的激光条纹图像，且后面的图像是对应高度上升的图像，不能是下降
    bool calibrateZtoLaser(vector <cv::Mat> &laserImg, const vector<double> zdis);
	//设置激光条纹参考平面,输入的refLaserImg必须为整条直线的激光条纹图像
	void setLaserRefPlane(Mat refLaserImg);
	//标定求解单应矩阵所需的对应关系
    ...
    //保存参数
    bool writeParameter(const string filePath);
	void setScanDir(ScanDir scanDir){ m_scanDir = scanDir; }
