...
//保存扫描头设置参数——蔡永凯
void XMLWriter::WriteSettingPara(std::string filename, laserScannerSetting& setParam)
{
	char text[1000];
	char* segmentation = " ";
	TiXmlDocument doc;
	...
	TiXmlElement *SettingPara = new TiXmlElement("SettingPara");
	doc.LinkEndChild(SettingPara);
	TiXmlElement *exposureTime_element = new TiXmlElement("exposureTime");
	SettingPara->LinkEndChild(exposureTime_element);
	sprintf(text, "%lf", setParam.exposureTime);
	TiXmlText * exposureTime_text = new TiXmlText(text);
	exposureTime_element->LinkEndChild(exposureTime_text);

	TiXmlElement *recontructionTimeOut_element = new TiXmlElement("recontructionTimeOut");
	SettingPara->LinkEndChild(recontructionTimeOut_element);
	sprintf(text, "%d", setParam.recontructionTimeOut);
	TiXmlText * recontructionTimeOut_text = new TiXmlText(text);
	recontructionTimeOut_element->LinkEndChild(recontructionTimeOut_text);

	TiXmlElement *FrameRate_element = new TiXmlElement("FrameRate");
	SettingPara->LinkEndChild(FrameRate_element);
	sprintf(text, "%d", setParam.FrameRate);
	TiXmlText * FrameRate_text = new TiXmlText(text);
	FrameRate_element->LinkEndChild(FrameRate_text);

	TiXmlElement *captureTime_element = new TiXmlElement("captureTime");
	SettingPara->LinkEndChild(captureTime_element);
	sprintf(text, "%lf", setParam.captureTime);
	TiXmlText * captureTime_text = new TiXmlText(text);
	captureTime_element->LinkEndChild(captureTime_text);

	TiXmlElement *moveSpeed_element = new TiXmlElement("moveSpeed");
	SettingPara->LinkEndChild(moveSpeed_element);
	sprintf(text, "%lf", setParam.moveSpeed);
	TiXmlText * moveSpeed_text = new TiXmlText(text);
	moveSpeed_element->LinkEndChild(moveSpeed_text);

	TiXmlElement *encoderSpace_element = new TiXmlElement("encoderSpace");
	SettingPara->LinkEndChild(encoderSpace_element);
	sprintf(text, "%d", setParam.encoderSpace);
	TiXmlText * encoderSpace_text = new TiXmlText(text);
	encoderSpace_element->LinkEndChild(encoderSpace_text);

	TiXmlElement *stepDis_element = new TiXmlElement("stepDis");
	SettingPara->LinkEndChild(stepDis_element);
	sprintf(text, "%lf", setParam.stepDis);
	TiXmlText * stepDis_text = new TiXmlText(text);
	stepDis_element->LinkEndChild(stepDis_text);

	TiXmlElement *triggerNum_element = new TiXmlElement("triggerNum");
	SettingPara->LinkEndChild(triggerNum_element);
	sprintf(text, "%d", setParam.triggerNum);
	TiXmlText * triggerNum_text = new TiXmlText(text);
	triggerNum_element->LinkEndChild(triggerNum_text);

	TiXmlElement *startPositon_element = new TiXmlElement("startPositon");
	SettingPara->LinkEndChild(startPositon_element);
	sprintf(text, "%d", setParam.startPositon);
	TiXmlText * startPositon_text = new TiXmlText(text);
	startPositon_element->LinkEndChild(startPositon_text);
...
void XMLWriter::WriteRT33(std::string filename, const vector<Mat>& Outputs)
{
	char text[1000];
	char* segmentation = " ";
	TiXmlDocument doc;
	...
	TiXmlElement * Image_RT_element = new TiXmlElement("MatRT33");
	Image_RT_element->SetAttribute("size", Outputs.size());
	doc.LinkEndChild(Image_RT_element);
	...
	for (int i = 0; i < Outputs.size(); i++)
	{
		...
		sprintf(text, "%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f%s%f"
			, Outputs[i].at<double>(0, 0)
			, segmentation
			, Outputs[i].at<double>(0, 1)
			, segmentation
			, Outputs[i].at<double>(0, 2)
